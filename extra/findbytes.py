import sys

ebcdic = ""
ascii = ""
n = 2

aline = []
eline = []
string = ''
for i in range(0, len(ascii), n):
        aline.append(ascii[i:i+n])
        eline.append(ebcdic[i:i+n])

final_form = ''
xored = ''

for arg in sys.argv[1:]:

    start = b'\x99'
    xord = int.from_bytes(start, byteorder="big") ^ int.from_bytes(bytes.fromhex(arg), byteorder="big")
    xstring = f'{xord:x}'.upper()
    if xstring in eline:
        print("{} ^ {} = {}".format(arg,start.hex(), xstring))
        final_form += start.hex()
        xored += xstring
    else:
        print('ruhroh')

print("Shellcode: {}\nXOR Key  : {}".format(xored, final_form))